...
(deftest redirect-unauthenticated-user-to-authentication-test
  (let [routes   (create-routes 8080)
    ...
    (testing "unauthenticated user should be redirected with http code 303"
      ...
      (is (= ((get-in response [:response :headers]) "Location") "https://localhost:8080/cas-oppija/login?locale=fi&service=http://yki.localhost:8080/yki/auth/callback/fi/ADMISSION?examSessionId=&registrationId=")))))
