...
(defprotocol Evaluations
  ...
  (create-evaluation-payment! [db payment-helper payment redirect-urls])
...
(extend-protocol Evaluations
  ...
  (create-evaluation-payment!
    [{:keys [spec]} payment-helper payment redirect-urls]
    (jdbc/with-db-transaction [tx spec]
      (rollback-on-exception
        tx
        #(let [order-number-seq (:nextval (first (q/select-next-evaluation-order-number-suffix tx)))
           ...
           (insert-initial-payment-data! payment-helper tx (assoc payment :order_number order-number) redirect-urls)))))
