...
(defn evaluation-payment-receipt-contents [payment-helper pdf-renderer receipt-language template-data]
  ...
  (let [receipt-id (:order_number template-data)]
    (pgq/put email-q
             {:recipients [(:email template-data)]
              ...
              (when payment-helper
                [{:name        (str receipt-id ".pdf")
                  :data        (evaluation-payment-receipt-contents payment-helper pdf-renderer email-language (assoc template-data :receipt_id receipt-id))
                  :contentType "application/pdf"}])})))

(defn send-kirjaamo-evaluation-registration-completed-email! [email-q email-language order-time kirjaamo-email template-data]
  (let [template-with-subject (-> (assoc template-data :subject "YKI,")
                                  (update :subtests #(template-util/get-subtests % "fi")))
