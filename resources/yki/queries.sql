...
SELECT o.oid,
       ...
       (SELECT array_to_json(array_agg(lang))
        FROM (SELECT language_code, level_code
              FROM exam_language
              WHERE organizer_id = o.id) lang) AS languages
FROM organizer o
...
UPDATE organizer
SET agreement_start_date = :agreement_start_date,
...
INSERT INTO exam_language (language_code,
                           level_code,
                           organizer_id)
...
INSERT INTO quarantine (language_code,
                        start_date,
                        end_date,
                        birthdate,
                        ssn,
...
INSERT INTO quarantine_review (quarantine_id,
                               registration_id,
                               quarantined,
                               reviewer_oid)
...
ON CONFLICT ON CONSTRAINT quarantine_review_unique_quarantine_registration_combination
    DO UPDATE SET quarantined  = :quarantined,
...
SET state =
        CASE
            WHEN state = 'COMPLETED'::registration_state THEN 'PAID_AND_CANCELLED'::registration_state
            ELSE 'CANCELLED'::registration_state
            END
WHERE id = :id
  AND state NOT IN ('CANCELLED', 'PAID_AND_CANCELLED');
...
VALUES ((SELECT id
         ...
         WHERE oid = :oid
           AND deleted_at IS NULL
           AND agreement_end_date >= :session_date
        ...
        (SELECT language_code
         ...
         WHERE el.organizer_id = (SELECT id
                                  FROM organizer
                                  WHERE oid = :oid
                                    AND deleted_at IS NULL
                                    AND current_timestamp BETWEEN agreement_start_date AND agreement_end_date)
...
INSERT INTO exam_session_location(name,
                                  street_address,
                                  post_office,
                                  zip,
                                  other_location_info,
...
SELECT e.id,
       ...
       (SELECT array_to_json(array_agg(contact_row))
        FROM (SELECT name,
                     ...
                     phone_number
              FROM contact co
              WHERE co.id = (SELECT esc.contact_id
                             FROM exam_session_contact esc
                             WHERE esc.exam_session_id = e.id
                               AND deleted_at IS NULL
                             LIMIT 1)
                AND deleted_at IS NULL) contact_row)                                as contact,
       (SELECT array_to_json(array_agg(loc))
        FROM (SELECT name,
                     street_address,
                     post_office,
...
FROM exam_session es
         ...
         LEFT JOIN exam_session ies
                   ON ies.id <> es.id AND ies.level_code = es.level_code AND ies.language_code = es.language_code AND
                      ies.organizer_id = es.organizer_id
...
UPDATE exam_session
SET exam_date_id     = (SELECT id FROM exam_date WHERE exam_date = :session_date),
    language_code    =
        (SELECT language_code
         FROM exam_language el
         WHERE el.organizer_id = (SELECT id
...
INSERT INTO participant(external_user_id,
                        email)
...
INSERT INTO registration(state,
                         exam_session_id,
                         participant_id,
                         started_at,
                         kind)
...
SELECT NOT EXISTS (SELECT es.max_participants
                   FROM exam_session es
                   ...
                   HAVING (es.max_participants - COUNT(re.id)) <= 0) as exists;

-- name: select-not-registered-to-exam-session
SELECT NOT EXISTS (SELECT es.id
                   FROM exam_session es
                            INNER JOIN registration re ON es.id = re.exam_session_id
                   WHERE re.participant_id = :participant_id
                     AND re.state IN ('COMPLETED', 'SUBMITTED', 'STARTED')
...
FROM registration
WHERE state = 'STARTED'
  AND (started_at + interval '30 minutes') < current_timestamp;
...
SET state    = 'EXPIRED',
    modified = current_timestamp
WHERE id IN (:ids)
  AND state IN ('STARTED', 'SUBMITTED');
...
INSERT INTO exam_payment_new(state,
                             registration_id,
                             amount,
                             reference,
                             transaction_id,
...
SET state    =
        CASE
            WHEN state = 'SUBMITTED'::registration_state THEN 'COMPLETED'::registration_state
            ELSE 'PAID_AND_CANCELLED'::registration_state
            END,
    modified = current_timestamp
WHERE id = :id
  AND state IN ('SUBMITTED', 'EXPIRED', 'CANCELLED');
...
INSERT INTO cas_ticketstore (ticket)
VALUES (:ticket);
...
INSERT INTO cas_oppija_ticketstore (ticket)
VALUES (:ticket);
...
INSERT INTO participant_sync_status(exam_session_id)
...
INSERT INTO participant_sync_status(relocated_at,
                                    exam_session_id)
...
FROM exam_date ed
WHERE ed.exam_date >= COALESCE(:from, current_date)
  AND ed.deleted_at IS NULL
ORDER BY ed.exam_date ASC;

-- name: insert-exam-date<!
INSERT INTO exam_date (exam_date,
...
INSERT INTO exam_date_language(exam_date_id,
                               language_code,
                               level_code)
VALUES (:exam_date_id,
        :language_code,
...
WHERE edl.exam_date_id = :exam_date_id
...
--name: insert-contact<!
INSERT INTO contact (organizer_id,
                     name,
                     email,
                     phone_number)
VALUES ((SELECT id
...
INSERT INTO exam_session_contact (exam_session_id,
                                  contact_id)
VALUES (:exam_session_id,
        :contact_id);
...
FROM exam_session_contact esc
WHERE esc.exam_session_id = :exam_session_id
  AND esc.contact_id = (SELECT con.id
                        FROM contact con
                        WHERE con.name = :name
                          AND con.email = :email
                          AND con.phone_number = :phone_number
...
INSERT INTO evaluation (exam_date_id,
...
SELECT ev.id,
       ...
       ev.evaluation_end_date
FROM evaluation ev
WHERE ev.exam_date_id = :exam_date_id
  AND ev.deleted_at IS NULL;

--name: insert-evaluation-order<!
INSERT INTO evaluation_order (evaluation_id,
                              first_names,
                              last_name,
                              email,
                              birthdate)
...
INSERT INTO evaluation_order_subtest (evaluation_order_id,
                                      subtest)
VALUES (:evaluation_order_id,
        :subtest);
...
INSERT INTO evaluation_payment_new(state,
                                   evaluation_order_id,
                                   amount,
                                   reference,
                                   transaction_id,
...
SELECT eo.id,
       ...
       (SELECT array_to_json(array_agg(subtest))
        FROM (SELECT subtest
              ...
              WHERE evaluation_order_id = eo.id) subtest) AS subtests
...
WHERE (date_trunc('day', :from_inclusive) AT TIME ZONE 'Europe/Helsinki')::DATE <= epn.paid_at
  AND epn.paid_at < (date_trunc('day', :to_exclusive) AT TIME ZONE 'Europe/Helsinki')::DATE;

-- name: select-unpaid-new-exam-payments-by-registration-id
SELECT epn.href
FROM exam_payment_new epn
WHERE epn.registration_id = :registration_id
  AND epn.state = 'UNPAID';
